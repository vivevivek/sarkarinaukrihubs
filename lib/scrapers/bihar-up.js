...
// ─────────────────────────────────────────────────────────────────────────────
// 7. SBPDCL / NBPDCL — Bihar Power Distribution Companies
//    SBPDCL : https://www.sbpdcl.co.in
//    NBPDCL : https://www.nbpdcl.co.in
//    Posts  : Junior Lineman, Technician, Office Assistant
// ─────────────────────────────────────────────────────────────────────────────
export async function fetchBiharPower() {
  const portals = [
    { base: 'https://www.sbpdcl.co.in', name: 'SBPDCL', badge: 'Official' },
    { base: 'https://www.nbpdcl.co.in', name: 'NBPDCL', badge: 'Official' },
  ];
  const all = [];
  for (const { base, name } of portals) {
    try {
      const jobs = await scrapeNICNoticeBoard(base, base, name, 'PSU', 'Bihar');
      all.push(...jobs);
...
// ─────────────────────────────────────────────────────────────────────────────
// 11. UPSESSB — UP Secondary Education Service Selection Board
//     Portal : https://upsessb.org
//     Posts  : TGT, PGT, LT Grade Teacher
// ─────────────────────────────────────────────────────────────────────────────
export async function fetchUPSESSB() {
  const BASE = 'https://upsessb.org';
  return scrapeNICNoticeBoard(BASE, BASE, 'UPSESSB', 'Teaching', 'Uttar Pradesh');
}

// ─────────────────────────────────────────────────────────────────────────────
// 12. UPBEB — UP Basic Education Board (Primary Teacher recruitment)
//     Portal : https://upbasiceducationboard.gov.in
//     Posts  : Primary Teacher via BTC/D.El.Ed
// ─────────────────────────────────────────────────────────────────────────────
export async function fetchUPBEB() {
  const BASE = 'https://upbasiceducationboard.gov.in';
  return scrapeNICNoticeBoard(BASE, BASE, 'UPBEB', 'Teaching', 'Uttar Pradesh');
}

// ─────────────────────────────────────────────────────────────────────────────
// 13. NHM UP — UP National Health Mission
//     Portal : https://upnrhm.gov.in
//     Posts  : ANM, CHO, Staff Nurse, ASHA Facilitator, Lab Technician
// ─────────────────────────────────────────────────────────────────────────────
export async function fetchNHMUP() {
  const BASE = 'https://upnrhm.gov.in';
  return scrapeNICNoticeBoard(BASE, BASE, 'NHM UP', 'Health', 'Uttar Pradesh');
}

// ─────────────────────────────────────────────────────────────────────────────
// 14. UPPCL — UP Power Corporation Limited
//     Portal : https://www.uppcl.org
//     Posts  : JE (Electrical), ARO, Revenue Inspector, Office Assistant
// ─────────────────────────────────────────────────────────────────────────────
export async function fetchUPPCL() {
  const BASE = 'https://www.uppcl.org';
  const $    = await fetchHtml(BASE);
  const jobs = [];
  ...
  $('a').each((i, el) => {
    ...
    if (
      ...
      jobs.push(makeJob({
        ...
        category: 'PSU',
        section:  guessSection(text),
        organization: 'UP Power Corporation Limited',
        state:    'Uttar Pradesh',
      }));
    }
  });
